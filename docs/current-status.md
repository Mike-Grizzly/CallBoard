...
**Session 2026-06-11 (social sign-in — Google + Apple, branch `claude/trusting-mccarthy-ieymt6`, NOT merged / not device-verified):** added one-click OAuth on `/login` and `/signup`. New `signInWithOAuth()` server action (`app/actions/auth.ts`) requests the provider's authorization URL via `supabase.auth.signInWithOAuth` (PKCE) and redirects to it; the existing `/auth/callback` route already exchanges the code for a session, so it was unchanged. New shared client component `components/auth/oauth-buttons.tsx` (Google + Apple buttons with per-button `useFormStatus` pending, "or continue with" divider) rendered on both pages — Supabase resolves an existing user to sign-in and a new one to signup, so the same buttons serve both. OAuth users carry no `first_name`/`last_name` metadata, so `lib/auth.ts` gained `deriveName()` which falls back to Google's `given_name`/`family_name` or splits a `full_name`/`name`; profile + org auto-creation is otherwise identical to email/password signup. Login page now surfaces `?error=oauth|auth_callback`. New `.auth-divider`/`.auth-oauth*` styles. **Setup the user owns:** enable Google + Apple providers in the Supabase dashboard (client IDs/secrets, provider redirect = `<project-ref>.supabase.co/auth/v1/callback`, app callback `${NEXT_PUBLIC_SITE_URL}/auth/callback` in allowed redirect URLs); Apple needs an Apple Developer membership. `tsc` + `eslint` clean. Not browser-verified. Detail: `feature-specs/02-auth.md`.
