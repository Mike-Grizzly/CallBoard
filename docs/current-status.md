...
**Session 2026-06-15 (ETC Eos integration — PROTOTYPE, branch `claude/optimistic-knuth-dvrzxz`, NOT browser/hardware-verified):** bridging the script-cue tool to **ETC Eos**, in two halves, built to work from the **Vercel-hosted https site** (not just local dev). (1) **Cue export to Eos** — Eos imports cue lists via the **USITT ASCII Showfile** format (`File ▸ Import ▸ USITT ASCII`), *not* CSV, so the existing CSV export stays for humans and a new `features/scripts/eos-ascii.ts` (`cueSheetToEosAscii`) emits a `.asc` showfile (`Ident 3:0`/`Manufacturer ETC`/`Console Eos`, `Clear All`, `Cue <n>`/`Text <label>` blocks, sorted numeric-aware; number+label only, no levels/fades). New **"Export for Eos"** button beside "Export CSV"; works on Vercel today (pure client download). (2) **Live follow (one-way, Eos → Proscene), routed through Supabase Realtime** — a browser can't read Eos's OSC/UDP and an https page can't reach localhost (Chrome 142 Local Network Access), so cues go: Eos →(OSC/UDP)→ **a small Electron helper app on the booth computer** →(wss broadcast)→ **Supabase Realtime** →(wss)→ the Proscene browser. Both ends connect *outbound*, so it works from the hosted site. **Helper app** `tools/eos-helper/` (Electron): `main.js` (window + `dgram` OSC socket + IPC + mock generator), `preload.js` (contextIsolated bridge), `renderer/` (single status page — pairing-code box, OSC port, Start/Stop, two status dots, last cue, **Send test cues**, **Quit**), `osc.js` (OSC parser + active-cue extractor, handles bundles), `publisher.js` (decodes pairing code, opens Realtime channel via Node `ws` transport, broadcasts cues, re-sends last cue on resubscribe), `headless.js` (no-GUI variant, `--code`/`--osc-port`/`--mock`). **App side:** `features/scripts/eos-pairing.ts` (`channelForProduction`→`eos:<id>`; `buildPairingCode` = base64 of `{url,anonKey,channel}`); `use-eos-follow.ts` rewritten to subscribe via Supabase Realtime broadcast (`useEosFollow({enabled,channel,onCue})` + `eosCueMatches`), driving the existing jump path; the cue sheet's **"Follow Eos"** toggle now shows a **Copy pairing code** button + connecting/live/last-cue status (disabled with "Realtime not configured" if NEXT_PUBLIC env absent). **Settings:** new `/settings/eos-helper` page ("Lighting console (Eos helper)") — what it is, **Get the helper app** button, 3-step setup. No DB/auth/server-action changes. **Verified:** OSC extractor + pairing base64 round-trip pass unit tests; all helper JS syntax-checks; `tsc` clean (0 errors project-wide); `eslint` clean on new files (4 `script-viewer.tsx` warnings pre-existing). **NOT verified on real Eos hardware/Nomad, nor run as a packaged Electron app** — run from source for now; key follow-ups. Known limits: helper is unsigned/run-from-source (signed installer is a later step); broadcast channel `eos:<productionId>` uses the public anon key (fine for cue nav, no data; token-scoping is a follow-up); one-way only; Eos-only (grandMA3/ChamSys future). Replaced the earlier same-session `tools/eos-bridge` (localhost SSE, dev-only) — that approach couldn't work from Vercel (mixed-content/LNA). Spec: `feature-specs/21-eos-integration.md`; helper guide: `tools/eos-helper/README.md`.
