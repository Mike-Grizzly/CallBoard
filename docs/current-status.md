...
**Session 2026-06-16 (onboarding walkthroughs — round 3, same branch, NOT browser-verified):** testing feedback. (1) **Persistent "Take a tour" launcher** — the hub tour only auto-showed once (already-seen → never again, with no way back). New `components/tour/start-tour-button.tsx` (question-mark icon in the hub topbar-actions) dispatches a `proscene:start-tour` event; the controller resolves the current screen's tour and starts it fresh (a `runId` bump remounts at step 1) regardless of seen state. Because the hub layout wraps the overview + blocking + script pages, the one button serves all three. (2) **Blocking & script tool tours** — two new tours explaining each tool's key features: `blocking` (scenes & off-stage cast / the stage & beats / set pieces, notes, comments) and `script` (tool rail / mark-up page / bookmarks & cue-sheet CSV / page nav & zoom). `resolveTour` now routes `/productions/[slug]/blocking` and `/productions/[slug]/script` sub-paths to these before the hub fallback; anchors added to the three blocking panels (`bk-side-left/center/right`) and four script regions (`sv-tools/workspace/side/pagenav`). Both auto-show on first visit and replay from the launcher. Tour files lint clean; `tsc` clean; `next build` compiles. (Pre-existing eslint problems in `blocking-canvas.tsx`/`script-viewer.tsx` — a `Date.now` purity error, unused imports, `<img>` warnings — are untouched baseline, not from the `data-tour` additions.)

**Session 2026-06-16 (onboarding walkthroughs — round 2, same branch, NOT browser-verified):** two pieces of testing feedback. (1) **Tooltip clamping bug** — on the dashboard's last step (5/5, pointing at the bottom-left settings link) the card hung off the bottom of the window and was unclickable. Root cause: the clamp pinned the card's *top* to the viewport bottom, ignoring its height. `coach-tour.tsx` now measures the rendered card (`useLayoutEffect` + ref) and clamps so its full height fits, and the placement flip (below↔above) uses the measured height too. Dropped the `translateY(-100%)` trick in favour of computing the real top directly. (2) **First-login welcome modal** — before any tour auto-runs, the first dashboard visit shows a small "Want a quick tour?" prompt (`welcome-modal.tsx`): "Show me around" starts the dashboard tour (and lets later screens auto-tour), "I'll explore on my own" marks everything seen so nothing auto-pops. Gated on a new `intro` key in `tours_seen`; new `dismissAllTours()` action backs the decline, and `resetAllTours()` now preserves the `intro` key so a manual replay doesn't re-show the welcome modal. Also swapped both portals to the repo's `typeof document === "undefined"` SSR guard (matching `confirm-dialog.tsx`) instead of a `mounted` flag. `tsc`/`eslint` clean; `next build` compiles. Still needs a live eyeball.

**Session 2026-06-16 (onboarding walkthroughs — coachmark tours, branch `claude/tender-carson-u08lyc`, NOT browser-verified):** built a "tour guide" onboarding system — spotlight coachmark pop-ups that walk a user through a screen the first time they hit it, replayable from Settings. (1) **Engine** — `components/tour/coach-tour.tsx` portals a dim layer with a cut-out ring over an anchored element plus a tooltip card (Back/Next/Skip, step counter, keyboard nav, scroll/resize tracking, clamps tooltips to the viewport). `components/tour/tour-controller.tsx` (mounted once in `app/(app)/layout.tsx`) is pathname-aware: auto-starts a tour the first time its screen is reached, force-replays on `?tour=1` (then strips the param), and persists "seen" on close. (2) **State** — new `profiles.tours_seen text[]` (migration `add_tours_seen_to_profiles`, applied via Supabase MCP, **not** `db:push`), surfaced on `CurrentUser.toursSeen`. `features/tours/actions.ts` has `markTourSeen` (idempotent append) + `resetAllTours`. (3) **Steps** — `features/tours/steps.ts` (pure constants, not a `"use server"` file) defines three tours: `dashboard`, `productions` (list), `production-hub`; `resolveTour(pathname)` maps screens (`/productions/new` excluded). **Role-aware for free:** steps point at `data-tour` anchors and capability-gated UI isn't rendered for roles that lack it, so those steps are skipped — no role logic in the tour. (4) **Anchors** added to the dashboard (greeting/chips/hero), the rail (productions section + settings link), the productions list (heading/new/cards), and the hub (title/tabs/actions). (5) **Replay** — a "Walkthroughs" card on Settings (`tour-replay.tsx`) with "Replay all" (clears seen → dashboard tour) + per-screen links. (6) **CSS** — `.tour-*` block in `globals.css` (z 1400–1402, above the confirm dialog). `tsc` clean; `eslint` zero net problems (2 pre-existing `<img>` warnings in `rail.tsx`, untouched); `next build` compiles + emits the full route table. **Desktop-first:** dashboard anchors live in the desktop command center, so the dashboard tour is a no-op at phone widths (absent anchors are skipped). Spec: `feature-specs/21-onboarding-walkthroughs.md`. **Not browser-verified** (no display in sandbox) — needs an eyeball on spotlight positioning, tooltip placement across screens, and the replay flow.
