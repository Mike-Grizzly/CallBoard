...
**▶ RESUME HERE — Billing go-live readiness (open):** Studio + org billing are code-complete on PR #66 (draft) and the **sandbox money loop is now live-verified end-to-end** (see session 2026-07-02) — but **NOT merged**; production is still on `main` with billing paused (`BILLING_ENABLED=false` there). Remaining checklist:
1. ~~Wire a Stripe test webhook → the preview URL~~ **DONE 2026-07-01/02:** a second test-mode event destination targets the branch-alias preview URL (4 events), with its signing secret in the **Preview-scoped** `STRIPE_WEBHOOK_SECRET`; the production destination/secret is untouched. Deliveries 200, DB syncs confirmed.
2. ~~Run the sandbox cycle~~ **DONE 2026-07-02:** verified live — Studio checkout→sync; immediate cancel→access ends instantly (`ended_at`); lapsed→Single Tool/Script resubscribe (customer reused) **with a promo code**; per-tool gate; two-step confirm upgrade (real proration preview shown, then charge)→`studio` in-place swap. Remaining nice-to-have: an annual-interval purchase. **NEW & untested:** the designer→full-app conversion (`/focus/full-app`, added later on 2026-07-02) — test: convert, land on dashboard, org trial stamped from today, Studio seat shows `cancel_at_period_end` in Stripe, dashboard features open.
3. **Browser-verify:** Focus paywall, lock modal (re-check with confirm step), Focus settings billing (incl. new canceled-in-grace resubscribe card), marketing Studio CTAs.
4. **Security pass — manual (2026-07-02) + adversarial multi-agent (2026-07-03).** Manual pass fixed two LOW items (`changeDesignerPlan` → `payment_behavior:"error_if_incomplete"`; `attachWizardScriptByPath` → `assertCanMutate(..,"script")`). The **multi-agent pass then caught a real MEDIUM/HIGH bypass the manual pass missed** — `upgradeToFullApp` trial-farming (no admin check, no set-once guard on the org trial write; farmable via free designer sock-puppets + cross-org membership). **FIXED:** admin (`settings:manage`) check + set-once `isNull(trialStartedAt)` guard, and designer production-creates no longer start the org trial (so conversion still grants a full 60 days). See `decision-log.md` 2026-07-03. **Guard-axis issue — RESOLVED with the `is_personal_workspace` flag (2026-07-03):** owner chose the Canva/Monday model (designers can be working members of paid orgs, gated by that org; their own seat-gated work lives in a separate personal workspace). New `organizations.is_personal_workspace` column (migration applied) drives `seatGoverns()` in `guard.ts` — seat axis only in the designer's own flagged workspace, org billing everywhere else; `assertCanCreateProduction` structurally blocks designers from creating productions inside a company org; `startTrialIfFirstProduction` skips personal workspaces; `upgradeToFullApp` flips the flag on conversion. Only the designer↔company **workspace toggle UI** remains (server-side entitlement done) — see `open-questions.md`. Live key/webhook handling still to be exercised at go-live.
5. **Merge PR #66** (CI-gated), then **live Stripe:** live keys, live webhook, **portal plan-switching enabled** (Studio/Studio Pro holders have no in-app tier switch — the portal is their only up/downgrade path), wire the discounts (founding + 15% coupons).
6. **Flip to prod deliberately** — the trial clock + daily lifecycle cron go live for real orgs the moment `BILLING_ENABLED=true` ships to production.
