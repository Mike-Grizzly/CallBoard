...
**Session 2026-06-11 (social sign-in — Google, branch `claude/trusting-mccarthy-ieymt6`, NOT merged / not device-verified):** added one-click Google OAuth on `/login` and `/signup`. New `signInWithOAuth()` server action (`app/actions/auth.ts`) requests the provider's authorization URL via `supabase.auth.signInWithOAuth` (PKCE) and redirects to it; the existing `/auth/callback` route already exchanges the code for a session, so it was unchanged. New shared client component `components/auth/oauth-buttons.tsx` ("Continue with Google" button, per-button `useFormStatus` pending, "or continue with" divider) rendered on both pages — Supabase resolves an existing user to sign-in and a new one to signup, so the same button serves both. OAuth users carry no `first_name`/`last_name` metadata, so `lib/auth.ts` gained `deriveName()` which falls back to Google's `given_name`/`family_name` or splits a `full_name`/`name`; profile + org auto-creation is otherwise identical to email/password signup. Login page now surfaces `?error=oauth|auth_callback`. New `.auth-divider`/`.auth-oauth*` styles. **Apple held off** (needs paid Apple Developer membership) — provider-agnostic wiring, re-add via `OAUTH_PROVIDERS` + a second button. **Setup the user owns:** enable the Google provider in the Supabase dashboard (Google OAuth client ID/secret, authorized redirect URI `https://avqgfzrcwegebtbvmcwo.supabase.co/auth/v1/callback`, app callback `${NEXT_PUBLIC_SITE_URL}/auth/callback` in allowed redirect URLs). `tsc` + `eslint` clean. Not browser-verified. Detail: `feature-specs/02-auth.md`.
