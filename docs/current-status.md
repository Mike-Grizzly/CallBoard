...
**Session 2026-06-15 (ETC Eos integration — PROTOTYPE, branch `claude/optimistic-knuth-dvrzxz`, NOT browser/hardware-verified):** first cut of bridging the script-cue tool to **ETC Eos**, in two halves. (1) **Cue export to Eos** — Eos imports cue lists via the **USITT ASCII Showfile** format (`File ▸ Import ▸ USITT ASCII`), *not* CSV, so the existing CSV export stays for humans and a new `features/scripts/eos-ascii.ts` (`cueSheetToEosAscii`) emits a `.asc` showfile: `Ident 3:0`/`Manufacturer ETC`/`Console Eos` header, `Clear All`, then `Cue <n>`/`Text <label>` blocks (sorted numeric-aware; deliberately number+label only, no levels/fades, so it seeds the LD's stack without touching their looks). New **"Export for Eos"** button beside "Export CSV" in the cue sheet (`exportCueSheetEosAscii`). (2) **Live follow (one-way, Eos → CallBoard)** — a browser can't read the UDP/OSC Eos broadcasts, so a zero-dependency Node bridge (`tools/eos-bridge/eos-bridge.mjs`, `dgram` + `http`) listens for Eos OSC (`/eos/out/active/cue/<list>/<cue>` + `/…/cue/text`, incl. bundle unpacking) on UDP :8000 and re-emits the active cue as **SSE** (chosen over WebSocket: one-way, auto-reconnect, no deps) on `http://localhost:8080/eos`; `--mock` cycles fake cues to demo the loop with no console. Client hook `features/scripts/use-eos-follow.ts` (`useEosFollow` + `eosCueMatches`, matching by cue number — exact then numeric) drives the existing jump path (`setViewMode`/`setCurrentPage`/`setSelectedId`); a **"Follow Eos"** toggle + bridge-URL input in the cue sheet (off by default, opt-in per session) with a connecting/live/last-cue status line. No DB/auth/server-action changes — additive client code + standalone dev tooling. **Verified:** bridge runs, streams SSE, and correctly parses crafted Eos OSC packets (mock + UDP send tests); `tsc` clean (0 errors project-wide), `eslint` clean on the new files (the 4 `script-viewer.tsx` warnings are pre-existing). **NOT verified on real Eos hardware/Nomad** (ASCII import idents + OSC port config) — that's the key follow-up. Known limits: mixed-content blocks SSE from an https page to http://localhost (built for `next dev`); one-way only (two-way needs strict per-LD permissions); Eos-only for now (grandMA3/ChamSys are OSC-capable future targets); eventual "just connect your device" UX folds the bridge into a native app. Spec: `feature-specs/21-eos-integration.md`; run guide: `tools/eos-bridge/README.md`.
