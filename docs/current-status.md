...
**Session 2026-06-08 (monetization model — docs only, branch `claude/stoic-noether-2DYyd`):** decided the plan/free-trial model and captured it as a design spec — no code yet. Free tier = **one fully-featured production**, editable for a **fixed 60 days anchored to the first production's creation** (deliberately *not* the closing date, which would be gameable via far-out or nudged dates). Creating a second production requires the paid **Company** plan. Funnel: day-30 "15% off" soft upsell (first term only), day-55 "trial ending" warning, day-60 production goes read-only (viewable forever). Proposed schema: `organizations.plan` + write-once `organizations.trial_started_at`; a new `features/billing/` module (`trialState`, centralized `canMutate`/`assertCanMutate` mutate guard, constants outside `"use server"`). Billing-provider (Stripe) integration and the daily cron for the nudges are **not yet designed**. Full spec in `feature-specs/18-billing-trial-gating.md`; rationale in `decision-log.md` (2026-06-08); open items in `open-questions.md` → Product/Billing. **Status: Planned.**
