...
- **Scheduler UX pass (2026-05-29):**
  ...
  - *Slide-in builder tray.* Creating a call now opens the form as a **slide-in tray overlaid on the calendar** (`call-tray.tsx`) — a right-side drawer on desktop, a bottom sheet on phones (matching the event drawer / day sheet). It lazily loads the production's cast via the `getCallTrayData` server action; on save the tray closes and the calendar refreshes in place (`router.refresh()`), no navigation. Scoped/single-production calendars open it directly; the multi-production workspace picks a production first, then opens the tray. The full-page `/calls/new` + `/calls/[id]/edit` routes still exist (deep links, edit); `createCall`/`updateCall` now return `{ success }` instead of redirecting so both the tray and the pages can decide what to do next.
