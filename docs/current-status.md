...
**Session 2026-06-11 (same-line cue stacking fix, branch `claude/cue-spacing`):** the auto two-lane stacking (#38) regressed the "two cues on the same line" case — same-line cues were spilling into the second lane (side-by-side at the same height, ordered by x so the higher number could land first) instead of dropping the second below the first. `stackCueLabels` now: (a) takes `cueNumber` and orders same-line cues by cue number (lower on top); (b) stacks cues that share a line bucket vertically in the **same** lane (drop-down), reserving the two-lane stagger for cues on *different* nearby lines (the dense-cluster case that works well). Applies to both the on-screen SVG and the PDF/canvas export (same helper). `tsc`/`eslint` clean (no new warnings). Not browser-verified.
