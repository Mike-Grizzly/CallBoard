...
**Session 2026-06-11 (cue labels read in cue-number order, branch `claude/cue-spacing`):** two reported cue-label problems, both rooted in the auto two-lane stacking (#38) ordering by anchor position: (a) two cues on one line rendered side-by-side (higher number could land first) instead of dropping the second below; (b) cues read out of numeric sequence down the margin (e.g. 10, 13, 12). Rewrote `stackCueLabels` to lay a margin out **by cue number** (numeric-aware, "2" < "10"): single column by default — each label at its anchor height, dropping down only to clear the previous, lower-numbered label so a higher number never sits above a lower one. A colliding label **fans into a second column only when it's closely followed by another cue below** (so stacking it straight down would run into that cue); an isolated same-line group just stacks vertically. So two cues on a line alone stack vertically, but a same-line cluster with something close beneath staggers into two columns — and numbers always read in order. Screen + PDF export share the helper. `tsc`/`eslint` clean (no new warnings). Not browser-verified; `FOLLOW` (look-ahead window) is tunable.
