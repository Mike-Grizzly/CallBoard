...
2. ~~Run the sandbox cycle~~ **MOSTLY DONE:** verified live — Studio checkout→sync, immediate cancel→`canceled`, lapsed→Single Tool/Script resubscribe (Stripe customer reused), per-tool gate, lock-modal upgrade→`studio_pro` (in-place price swap, same sub id, instant unlock). **Still to test in sandbox:** the NEW two-step confirm flow + proration preview (added 2026-07-02, after the tests above), a promo-code checkout, an annual-interval purchase, and `ended_at` cancel behavior.
