...
**Session 2026-06-11 (cue labels read in cue-number order, branch `claude/cue-spacing`):** two reported cue-label problems, both rooted in the auto two-lane stacking (#38): (a) two cues on one line rendered side-by-side (higher number could land first) instead of dropping the second below; (b) a cue inserted out of physical order (e.g. 13 anchored above 12) showed out of numeric sequence down the margin (10, 13, 12). Replaced the position-based two-lane placement in `stackCueLabels` with a **single column ordered by cue number**: each label sits at its anchor height but is pushed down only enough to never sit above the previous, lower-numbered label — so labels always read 10, 12, 13 top-to-bottom, and same-line cues stack vertically by number. Numeric-aware sort ("2" < "10"). `lane` is now always 0 (two-lane retired; renderers unchanged). Same helper drives screen + PDF export. `tsc`/`eslint` clean (no new warnings). Not browser-verified. Trade-off: dense clusters push down further than the two-lane layout did — revisit if crowding appears.
