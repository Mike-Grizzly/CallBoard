...
**Session 2026-06-13 (app-wide responsive pass, branch `claude/narrow-screen-overflow`, partially browser-verified on the Vercel preview):** the app was built responsive for **phones only** (a single `max-width: 720px` breakpoint) and assumed large-desktop everywhere else, so the middle band (~720–1300px: 4:3 laptops, split-screen, tablets) broke — fixed-width panels overflowed or collided and content-heavy grids pushed the page sideways. This pass attacks the three root causes (missing `min-width: 0`, fixed widths that can't shrink/wrap, only one breakpoint). **Header tabs now WRAP** to multiple rows instead of cutting off — this *supersedes* the horizontal-scroll approach described in the 2026-06-12 entry below (the user found scrolling unintuitive and wanted everything visible in one view). **Script viewer:** (1) the shell had `max-width:1440;margin:0 auto` but no `width`, so inside `.page` (a flex column) the auto margins overrode stretch and it sized to content, pushing the 248px bookmarks panel off the right edge — added `width:100%` so the canvas shrinks to fit (verified fixed); (2) the toolbar row now `flex-wrap`s so it drops a line instead of colliding with the bookmarks panel (verified); (3) on the tablet band (721–1100px) the pinned 248px side panel is collapsed and bookmarks **+ page notes** open via the existing floating button → bottom sheet (the `AnnotationsPanel` was added to that sheet so collapsing it loses nothing). **Global baseline:** every bare `1fr` grid track → `minmax(0, 1fr)` (calendar 7-col, people/production 6-up stats, calendar/day/notes/onboarding panels, the app shell) so content can shrink instead of blowing out; `overflow-x: clip` on `.app` as a no-horizontal-scroll invariant (wide-by-design regions — PDF viewer, data tables — keep their own scroll). **Collapsible side panels (new pattern):** the calendar sidebar (calendars + filters) and the day-view detail panel were `display:none` below 980px with **no way to reopen** — both now slide in as off-canvas drawers (sidebar from the left via a new `Layers` toolbar toggle; day panel from the right via a toggle on the timeline), each with a scrim, driven by a `data-side-open` attribute on `.cal`/`.day`, reusing the existing `.cal-scrim` styling (+ a `.cal-side-scrim` `min-width:981px` safety to drop a stray scrim on resize). The **notes** two-pane was left as-is — it already has a clean phone behavior (editor goes full-screen via `data-editor-open`) and only a modest 272px on tablet. `tsc` clean. **Verified by the user on the preview:** tab wrap, bookmarks-panel-on-screen, toolbar-no-collision, calendar sidebar drawer. **Not individually browser-verified:** the day-view drawer and the breadth of the `minmax`/`overflow-x` baseline across every screen. **Not merged yet** (branch only). See `decision-log.md` for the breakpoint/pattern decisions. **Deferred:** container-query refinements (only if a specific component misbehaves), and per-screen intermediate column-count tuning (the `minmax` baseline already prevents overflow).

**Session 2026-06-12 (narrow / 4:3 screen overflow — production header, branch `claude/narrow-screen-overflow`):** the production header's tab strip and action cluster only got their overflow protections (horizontal-scroll tab strip; wrapping action row) at the ≤720px phone breakpoint, so on a 4:3 MacBook (>720px but narrower than 16:9) the tab row and "Cast & Crew / New report" buttons ran off the right edge and pushed the page sideways. Moved those protections into the **base** rules so they apply at all widths: `.tabs` is always a contained horizontal scroller (negative `--pad-x` margins keep edge-to-edge alignment; scrollbar hidden), `.tab` never shrinks/wraps, and `.topbar-row`/`.topbar-actions` wrap when they don't fit. No change on wide screens (everything still fits on one line). This header is shared by every production sub-page, so the fix is broad. `tsc`/`eslint` clean. Not browser-verified. **Follow-up:** several data tables (reports list, documents list view, blocking) use fixed-px column grids that can still overflow on very narrow widths — left for a targeted pass once we confirm which screens the user actually hits.
