...
- **Scheduler UX pass (2026-05-29):**
  ...
  - *Week-scroll fix.* `.cal-main` was missing `min-width: 0`, so the fixed-px day columns blew the `1fr` track out to full content width — the week showed ~2 columns and wouldn't scroll sideways. Added `min-width: 0` down the `.cal-main → .week → .week-scroll` chain so the overflow happens inside the scroller (3/5/7 columns swipe correctly).
