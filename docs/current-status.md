...
4. **Security pass — manual (2026-07-02) + adversarial multi-agent (2026-07-03).** Manual pass fixed two LOW items (`changeDesignerPlan` → `payment_behavior:"error_if_incomplete"`; `attachWizardScriptByPath` → `assertCanMutate(..,"script")`). The **multi-agent pass then caught a real MEDIUM/HIGH bypass the manual pass missed** — `upgradeToFullApp` trial-farming (no admin check, no set-once guard on the org trial write; farmable via free designer sock-puppets + cross-org membership). **FIXED:** admin (`settings:manage`) check + set-once `isNull(trialStartedAt)` guard, and designer production-creates no longer start the org trial (so conversion still grants a full 60 days). See `decision-log.md` 2026-07-03. **Guard-axis issue — INTERIM HARDENING LANDED (2026-07-03):** owner decided designers can join companies; `orgIsCompany()` in `guard.ts` now gates a designer caller by the resource org's billing when it's a company (footprint: grandfathered / subscription / started-trial), by the seat only in their own workspace — closes the seat-write-into-lapsed-company face + the paid-company lock-out. The precise `is_personal_workspace` flag remains a scoped follow-up for when the designer→company join/switch UI is built (`open-questions.md`). Live key/webhook handling still to be exercised at go-live.
