...
**▶ RESUME HERE — Billing go-live readiness (open):** Studio + org billing are code-complete and **CI-green on PR #66 (draft)**, but **NOT yet validated end-to-end** and **NOT merged** — production is still on `main` with billing paused (`BILLING_ENABLED=false` there). The per-tool gate is the only thing live-tested (via a hand-set DB seat). Before going live, work this checklist:
1. **Wire a Stripe test webhook → the preview URL.** Today the only endpoint targets `www.proscene.app` (production, running `main` without the designer code), so **no purchase has ever synced** — this is the #1 gap.
2. **Run a full sandbox cycle:** subscribe each tier → webhook sync → gate behavior → **upgrade** (`changeDesignerPlan`) → downgrade → cancel → access-after-cancel → promo code.
3. **Browser-verify:** Focus paywall (`/focus` seatless), locked-tool upgrade modal, Focus settings billing, marketing Studio CTAs.
4. **Final security pass on the COMPLETE final diff.** Note: `changeDesignerPlan` (`features/designer/actions.ts` — `stripe.subscriptions.update` + immediate DB plan write) and the locked-tool modal (`features/designer/tool-lock.tsx`) were added **AFTER** the 2026-06-30 security review and are **not yet reviewed** (esp. the DB plan write happening even if the proration charge later fails). Re-check the org path + live key/webhook handling too.
5. **Merge PR #66** (CI-gated), then **live Stripe:** live keys, live webhook, portal plan-switching enabled, wire the discounts (founding + 15% coupons).
...
**Test-account note:** `mgrigsby.beazleyrealtors@gmail.com` was manually set to a `single_tool`/`script` seat via SQL to verify the gate — it has **no real Stripe subscription**, so the upgrade action returns "no active subscription." Reset for a clean start: `UPDATE profiles SET designer_plan=NULL, designer_tool=NULL, designer_subscription_status=NULL WHERE email='mgrigsby.beazleyrealtors@gmail.com';`. Open follow-ups (referral engine, solo-designer cast for Blocking, accurate-UI teasers, upgrade-interval edge): see `open-questions.md`.
