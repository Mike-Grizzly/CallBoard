...
4. **Final security pass on the COMPLETE final diff.** Added after the 2026-06-30 review and not yet reviewed: `changeDesignerPlan` + `previewDesignerPlanChange` + confirm-step lock modal (2026-07-02), **`upgradeToFullApp()` + `/focus/full-app`** (2026-07-02 — mode flip, trial re-stamp, seat wind-down), `requestSiteUrl()` return-URL derivation, the `ended_at` webhook change, and (from the PR #67 merge) `attachWizardScriptByPath` — that one files a script without its own billing-lock check (reachable only with a path that passed the gated upload; assess in the sweep). Re-check the org path + live key/webhook handling too.
