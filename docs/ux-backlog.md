...
### S1 · Shared `<Drawer>` primitive — **P1 · L** — 🟡 IN PROGRESS (claude/ux-backlog-batch-8/9, 2026-07-14; primitive built + trash & person drawers migrated, 3 to follow)
...
**Shipped (primitive + 1/5):** `components/ui/drawer/` — `<Drawer open onClose side width mobileSheet ariaLabel/ariaLabelledby dismissible>` owning portal, backdrop, Escape, body-scroll lock, focus trap + focus return (via S4's `useFocusTrap`), the enter/exit slide animation, and a ≤720px bottom-sheet mode. **All motion is single-source:** the duration lives in `drawer.constants.ts` (`DRAWER_DURATION_MS`, injected as the `--drawer-dur` CSS var so the JS unmount delay and the CSS transition can't drift), and the easing/snap, backdrop tint, slide, width, and sheet height are CSS variables in `drawer.css` — retuning any of them updates **every** drawer at once. Migrations so far: **`trash-drawer.tsx`** (batch 8) and **`person-drawer.tsx`** (batch 9) render through `<Drawer>` — each dropped its bespoke portal/backdrop/Escape/inline animation and gained the focus trap it lacked plus the mobile bottom-sheet. For person-drawer the old `.pp-drawer-wrap` backdrop was deleted and `.pp-drawer` collapsed to a content scroll region (the `pp-fade`/`pp-slide` keyframes stay — document-drawer + other overlays still reference them). Both screenshot-verified via harness at 1440px (480px right panel) and ≤720px (full-width bottom sheet). **Remaining (one PR each):** announcement-detail-drawer (ann-), event-drawer (cal-), document-drawer (the outlier — 1400px width + fullscreen mode + no sheet, migrate last).
...
**Shipped (partial):** reusable `useFocusTrap(active)` hook (`components/ui/use-focus-trap.ts`) — seeds focus into the overlay if it has none, traps Tab/Shift+Tab, returns focus to the trigger on close — applied to **`ConfirmDialog`**, which backs every destructive-action confirm app-wide, and to the shared **`<Drawer>`** primitive (S1), so **trash-drawer** (batch 8) and **person-drawer** (batch 9) now trap focus as they migrate. **Remaining:** the other drawers (documents/announcements/calls tray) trap focus as they migrate onto `<Drawer>` in the S1 one-per-PR series; editor-containing modals (dept-note, distribute preview) were deferred because a Tab-trap fights TipTap's own Tab handling — they need per-modal treatment.
