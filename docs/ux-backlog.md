...
### M2 · Marketing truth pass — unbacked feature claims — **P0 · M** — ✅ DONE (claude/ux-backlog-batch-6, 2026-07-14; owner decisions per-claim)
...
**Shipped — per-claim owner decisions:** **(a) conflict detection** — removed everywhere it was claimed as shipped (home hero demo + calendar section, features SM/calendar sections, people list); owner wants a real per-show conflict system later, logged in `open-questions.md`. **(b) drag-to-reschedule** — removed (possible future UX). **(c) works offline** — removed; reframed as the PWA "installs to your home screen / opens full-screen" story. **(d) native app** — it's a PWA ("Add to Home Screen"), so App Store/Google Play/"Mobile app" copy reworded to "installable web app," with a note that dedicated iOS/Android apps are on the roadmap (footer "Mobile app" link → FAQ). **(e) SMS reminders** — removed (not planned); push is scoped to announcements/@mentions (calls don't push). **(f) iCal subscribe** — removed. **(g) report PDF export** — reports are email-distribution only; report-PDF copy fixed (a report PDF export is roadmap). *Script & blocking PDF export is real and left intact.* **(h) cross-company script cache** — removed the "free for the next company" claim; reworded to the real within-company sharing (one parse, per-user private markup). **Bonus truth fixes found while editing:** "export a clean rehearsal schedule" (no schedule export) and calendar copy implying calls send notifications (they don't) — both softened to real capabilities (views, colour-coding, call confirmations, generate-with-skip-clashes). Screenshot-verified on home/features/pricing/FAQ.
...
### M11 · Marketing home hydration warnings — **P2 · S** — ✅ DONE (claude/m11-hydration, 2026-07-21)
...
**Root cause + fix:** it wasn't the Sanity swap — `HOME_HTML` (`home-content.ts`) had **one stray `</div>`** in the hero block (98 opens / 99 closes). The extra close prematurely closed the `dangerouslySetInnerHTML` wrapper on the server, so the browser's document parser spilled the following `<section>`s out as siblings — a DOM shape the client's `innerHTML` parse didn't reproduce, hence the mismatch (and the "Invalid HTML tag nesting" hint). Removing the one tag balanced the markup; reproduced on the dev server and confirmed **zero hydration/script console errors** afterward, page renders unchanged. (Method: the marketing pages render without a DB — ran `next dev` with a dead proxy so Sanity falls back to static, loaded `/` in headless Chromium, read the console.)
...
**Decision:** Owner chose to **keep the marketing site always-light** — a deliberate brand choice, not an accident. No code change. The batch-11/brand work already keeps marketing always-light even for dark-theme app users (it reads the canonical, never-themed accent). Recorded in `decision-log.md` (2026-07-21).
...
### S1 · Shared `<Drawer>` primitive — **P1 · L** — ✅ DONE (claude/ux-backlog-batch-8/9/10, 2026-07-14; primitive built + all 5 drawers migrated)
...
**Shipped (primitive + 1/5):** `components/ui/drawer/` — `<Drawer open onClose side width mobileSheet ariaLabel/ariaLabelledby dismissible>` owning portal, backdrop, Escape, body-scroll lock, focus trap + focus return (via S4's `useFocusTrap`), the enter/exit slide animation, and a ≤720px bottom-sheet mode. **All motion is single-source:** the duration lives in `drawer.constants.ts` (`DRAWER_DURATION_MS`, injected as the `--drawer-dur` CSS var so the JS unmount delay and the CSS transition can't drift), and the easing/snap, backdrop tint, slide, width, and sheet height are CSS variables in `drawer.css` — retuning any of them updates **every** drawer at once. **All five migrated:** `trash-drawer.tsx` (batch 8), `person-drawer.tsx` (batch 9), and `announcement-detail-drawer.tsx` + `calendar/event-drawer.tsx` + `documents/document-drawer.tsx` (batch 10). Each dropped its bespoke portal/backdrop/Escape/inline animation and gained the focus trap it lacked. Per-surface namespaces collapsed to content styles only: deleted `.pp-drawer-wrap`, `.ann-drawer-wrap` (+ its mobile sheet block + `ann-sheet-up` keyframe), `.cal-scrim` (+ `cal-drawer-in` keyframe) and the mobile `.cal-drawer` re-positioning; `.pp-drawer`/`.ann-drawer`/`.cal-drawer` are now just content scroll regions. The `pp-fade`/`pp-slide` keyframes stay (still referenced by other overlays). The document-drawer is the outlier — kept its fullscreen overlay (bumped to z-index 260 above the drawer root) and routes Escape/backdrop through a fullscreen-aware dismiss handler; `mobileSheet={false}` so the viewer stays a wide right panel on phones. All screenshot-verified via harness at 1440px and ≤720px (ann/cal → bottom sheet, doc → 92vw right panel).
...
**Shipped (partial):** reusable `useFocusTrap(active)` hook (`components/ui/use-focus-trap.ts`) — seeds focus into the overlay if it has none, traps Tab/Shift+Tab, returns focus to the trigger on close — applied to **`ConfirmDialog`**, which backs every destructive-action confirm app-wide, and to the shared **`<Drawer>`** primitive (S1), so **all five drawers** (trash/person/announcement/event/document — batches 8–10) now trap focus and return it on close. **Remaining:** the standalone calls tray isn't a `<Drawer>` yet (its own bottom-sheet component) — fold it in if it's ever reworked; editor-containing modals (dept-note, distribute preview) stay deferred because a Tab-trap fights TipTap's own Tab handling — they need per-modal treatment. editor-containing modals (dept-note, distribute preview) were deferred because a Tab-trap fights TipTap's own Tab handling — they need per-modal treatment.
