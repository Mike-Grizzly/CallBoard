...
- **Designer plan upgrade — one-click in-app upgrade built (2026-06-30).** The locked-tool modal's tier cards now call `changeDesignerPlan` (`features/designer/actions.ts`), which swaps the live subscription's price via `stripe.subscriptions.update` (prorated) and syncs the entitlement immediately so the tool unlocks. Remaining minor items: the upgrade interval defaults to monthly (doesn't preserve an annual subscriber's interval); it needs a real synced subscription to exercise (a manually-set test seat with no Stripe sub returns "no active subscription"); and a "Manage billing" link still routes to the portal as a fallback.
