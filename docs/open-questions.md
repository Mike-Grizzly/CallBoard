...
- **Designer → full-app ("level up") conversion path (added 2026-07-02, owner ask).** A designer-seat holder has no way to become a full-org customer. This is a CROSS-AXIS migration, not a price swap: the designer sub lives on the profile (personal Stripe customer) while org plans live on the organization (separate customer + subscription) — so a real conversion flow must start an org subscription, cancel/credit the designer seat, flip `access_mode` designer→full, and decide whether the 60-day org trial applies. The personal workspace is already an `organizations` row, so their productions carry over cleanly. **Decision:** do NOT put org tiers in the tool-lock modal (it's a focused unlock-this-tool moment); at most a "Running a company? See company plans" link in designer settings. Fold the real flow into the deferred **Accounts & entitlements model** scoping (above) — same problem space. Interim: concierge (support flips `access_mode`, user subscribes org plan from settings, cancels seat in portal).
- **Change card at the upgrade moment (added 2026-07-02, owner ask).** The in-app upgrade charges the subscription's default payment method with no way to switch cards mid-flow. Card management EXISTS today via the portal ("Manage billing" in both the lock modal and settings). Improvement: a "pay with a different card" link on the confirm panel that opens a portal `payment_method_update` flow (`billingPortal.sessions.create` with `flow_data`) returning to the locked tool. Small, contained; also decide dunning UX — if the upgrade's proration charge declines, the sub goes `past_due` (seat stays active during dunning) and our DB plan write has already granted the new tier (flagged in the go-live security pass).
