...
- **DECIDED 2026-07-03 (owner): designers CAN be working members of company orgs** (freelance designer invited to the companies they design for). So the fix direction is: gate a designer caller by the **resource org's billing** when acting in a company, and by their **seat** only in their own personal workspace. Implement with an explicit **`is_personal_workspace` flag** on `organizations` (set at designer signup; backfill existing) so the two are cleanly distinguished — cleaner than the billing-footprint heuristic, which mis-gates a brand-new company that hasn't started billing yet. Then `assertCanMutate/Operate/CreateProduction` (and the analogous read paths) branch on `org.isPersonalWorkspace ? seat : orgBilling` for designer callers. **Scoped as its own follow-up PR** (schema migration + signup wiring + guard rework + tests), to land alongside/before the designer→company join+switch UI so the residual never becomes UI-reachable. Not in PR #66.
  - **Interim option for PR #66 (pre-go-live hardening):** a billing-footprint heuristic in the designer guard branches (gate by org billing when the org is grandfathered / has a subscription / has a started trial, else the seat) closes the exploitable SECURITY face now with one benign brand-new-company edge; the flag-based version supersedes it. Pending owner go/no-go.
