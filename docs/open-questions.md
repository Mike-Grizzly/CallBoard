...
- **Designer plan upgrade path is portal-only.** The locked-tool upgrade modal (`features/designer/tool-lock.tsx`) "Upgrade my plan" CTA opens the Stripe customer portal — which only lets a Single Tool subscriber move up if **plan-switching is enabled in the portal config** with the Studio/Studio Pro prices added. A smoother one-click in-app upgrade would be a `changeDesignerPlan` action using `stripe.subscriptions.update` (swap the price item with proration); deferred.
