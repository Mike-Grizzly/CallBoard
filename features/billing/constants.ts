...
// Single site-wide switch for the entire trial + subscription system. When
// `true` (the live state) every paywall, the 60-day trial clock, Checkout /
// customer-portal, the trial banners/pills, and the daily billing-lifecycle
// cron are all ACTIVE. Setting it to `false` is a fully reversible kill switch
// (used during the open beta) that disables every paywall site-wide:
...
//     so re-enabling later gives orgs a fresh trial rather than an
...
//   • the marketing pricing page shows an open-beta "no payments yet" banner.
// Re-enabled 2026-06-29 to validate the Stripe workflow end-to-end in sandbox.
